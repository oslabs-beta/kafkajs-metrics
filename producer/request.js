...
function requestPendingDuration(producer, e) {
  ...
  if (
    ...
    if (producer.metrics.name) {
      console.log(
        `latency occuring in ${e.payload.apiName} for producer ${producer.metrics.name} (member Id: ${producer.metrics.memberId})`
      );
    } else {
      console.log(
        `latency occuring in ${e.payload.apiName} for producer (member Id: ${producer.metrics.memberId})`
      );
    }
  }
  ...
  if (
    ...
    if (producer.metrics.name) {
      console.warn(
        `BREAKPOINT ALERT: requestPendingDuration is above ${producer.metrics.options.requestPendingDuration.breakpoint} in ${e.payload.apiName} for producer ${producer.metrics.name} (member Id: ${producer.metrics.memberId})`
      );
    } else {
      console.warn(
        `BREAKPOINT ALERT: requestPendingDuration is above ${producer.metrics.options.requestPendingDuration.breakpoint} in ${e.payload.apiName} for producer (member Id: ${producer.metrics.memberId})`
      );
    }
  }
