...
function requestQueueSize(producer) {
  ...
  producer.on('producer.network.request_queue_size', (e) => {
    ...
    if (e.payload.queueSize > 5) {
      ...
      // console logs with/without producer name
      ...
      if (
        producer.metrics.requestQueueSize.logOn &&
        e.payload.queueSize >
          producer.metrics.options.requestPendingDuration.breakpoint
      )
        console.log(
          `ALERT: WARNING The requestQueueSize is above 5! Current requestQueueSize is : ${e.payload.queueSize} `,
